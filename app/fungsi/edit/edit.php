...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['barang'])) {
        ...
        $data[] = $tgl;
        $data[] = $id;
        $sql = 'UPDATE barang SET id_kategori=?, nama_barang=?, harga_beli=?, harga_jual=?, satuan_barang=?, stok=?, tgl_update=?  WHERE id_barang=?';
        $row = $config->prepare($sql);
        $row->execute($data);
    ...
    if (!empty($_GET['cari_barang'])) {
        ...
        if ($cari == '') {
            ...
            $sql = "SELECT barang.*, kategori.id_kategori, kategori.nama_kategori FROM barang INNER JOIN kategori ON barang.id_kategori = kategori.id_kategori WHERE barang.id_barang LIKE '%$cari%' OR barang.nama_barang LIKE '%$cari%';";
            $row = $config->prepare($sql);
            $row->execute();
            $hasil1 = $row->fetchAll();
?>
